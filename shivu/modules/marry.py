...
# 5. /divorce with confirmation
async def divorce(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    marriage = await user_marry_coll.find_one({
    ...
    keyboard = [[InlineKeyboardButton("Are You Sure 💔", callback_data=f"divorce:{user_id}")]]
    return await update.message.reply_text(
        "Are you sure you want to end this marriage?\nBoth partners must confirm.",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

# Confirm Divorce Callback
pending_divorces = set()

async def divorce_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    initiator_id = int(query.data.split(":")[1])

    marriage = await user_marry_coll.find_one({
        "$or": [{"user1": initiator_id, "user2": user_id}, {"user1": user_id, "user2": initiator_id}]
    })
    if not marriage:
        return await query.edit_message_text("You're not married.")

    key = frozenset([initiator_id, user_id])
    if key in pending_divorces:
        await user_marry_coll.delete_one({"_id": marriage["_id"]})
        pending_divorces.remove(key)
        return await query.edit_message_text("💔 Divorce complete. You are now single.")
    else:
        pending_divorces.add(key)
        return await query.edit_message_text("Confirmed by one partner. Waiting for the other...")

# 6. /wdivorce (no confirmation)
...
application.add_handler(CallbackQueryHandler(divorce_callback, pattern=r'^divorce:'))
